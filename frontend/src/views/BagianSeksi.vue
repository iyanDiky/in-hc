...
const initDataTable = () => {
  ...
  dataTableInstance = window.$('#bagianSeksiTable').DataTable({
    serverSide: true,
    processing: true,
    ajax: async function (data, callback, settings) {
      try {
        const response = await api.post('/bagian-seksi/datatables', data)
        callback({
  ...
  window.$('#bagianSeksiTable tbody').off('click', '.btn-edit')
  window.$('#bagianSeksiTable tbody').on('click', '.btn-edit', function() {
    const data = dataTableInstance.row(window.$(this).parents('tr')).data()
    openModal(data)
  })

  window.$('#bagianSeksiTable tbody').off('click', '.btn-delete')
  window.$('#bagianSeksiTable tbody').on('click', '.btn-delete', function() {
    const data = dataTableInstance.row(window.$(this).parents('tr')).data()
    deleteItem(data.id)
  })
}
